"""FastTasks / ``fastx_platform`` worker commands (optional dependency).

Imports ``fastx_platform.src.task`` at runtime. Missing packages produce a
...
def tasks_worker(concurrency: int) -> None:
    ...
    output.print_banner()
    try:
        from fastx_platform.src.task import Worker
    except ImportError:
...
def tasks_list() -> None:
    ...
    output.print_banner()
    try:
        from fastx_platform.src.task import TaskRegistry
    except ImportError:
        output.print_error(OPTIONAL_DEPS_FAST_PLATFORM_ERROR)
        return
