...
def test_dump_yaml_adds_trailing_newline() -> None:
    with patch("fastx_cli.commands.commit_history_setup.yaml.dump", return_value="x: 1"):
...
def test_precommit_pip_oserror(tmp_path: Path) -> None:
    ...
    pip.write_text("")
    with patch("fastx_cli.precommit.subprocess.run", side_effect=OSError("e")):
        ...
        with patch(
            ...
            with patch.object(FrameworkSourceLocator, "fastx_mvc_root", return_value=src):
                with patch.object(
                    ...
                    with patch(
                        "fastx_cli.project_generation.shutil.copy2",
