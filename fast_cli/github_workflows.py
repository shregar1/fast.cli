...
the monorepo checkout (parent of the ``fast_cli`` package). They are **not**
shipped inside the PyPI wheel; if the directory is missing, the copier warns and
returns ``False``.

Each copied file is passed through :class:`fast_cli.template_engine.TemplateRenderer`
...
class GitHubWorkflowsCopier:
    ...
    def copy_into_project(self, target_path: Path, context: dict) -> bool:
        ...
        context
            Template context for :class:`~fast_cli.template_engine.TemplateRenderer`.
